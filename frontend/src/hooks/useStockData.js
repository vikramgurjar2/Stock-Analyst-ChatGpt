...
export const useStockData = () => {
  ...
  useEffect(() => {
    ...
    if (token && !isInitializedRef.current) {
      ...
      stockUpdateSubscriptionRef.current = socketClient.subscribeToStockUpdates((data) => {
    ...
    return () => {
      if (stockUpdateSubscriptionRef.current) {
        socketClient.unsubscribeFromStockUpdates(stockUpdateSubscriptionRef.current);
      }
    };
  }, [selectedStock]);
  ...
  const fetchHistoricalData = useCallback(async (symbol, period = '1M') => {
    if (!symbol) return;
      ...
      if (watchlist.length > 0) {
        ...
        try {
          ...
          const watchlistWithQuotes = watchlist.map(stock => {
            ...
            return {
              ...stock,
              currentPrice: quote?.price || stock.price || Math.random() * 200 + 50,
          ...
          setPortfolioData(watchlistWithQuotes.map(stock => ({
            ...stock,
          ...
          setPortfolioData(watchlistWithMockPrices.map(stock => ({
            ...stock,
        ...
        const defaultWatchlist = [
          { symbol: 'AAPL', currentPrice: 165.50, change: 2.5 },
          { symbol: 'GOOGL', currentPrice: 2750.00, change: -1.2 },
        ...
        setPortfolioData(defaultWatchlist.map(stock => ({
          ...stock,
          shares: Math.floor(Math.random() * 200 + 50)
        })));
      }
